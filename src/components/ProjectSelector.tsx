...
export function ProjectSelector({ sectionLabel }: ProjectSelectorProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full py-16 px-6">
      ...
      <p className="text-slate-100 font-[Nunito_Sans] font-bold text-base mb-1">
      ...
      <div className="w-full max-w-sm space-y-2">
        {projects.map(project => (
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-slate-900 border border-slate-800 hover:border-red-500/40 hover:bg-slate-800/80 transition-all group text-left"
          >
            <div className="w-8 h-8 rounded-lg bg-red-500/10 flex items-center justify-center flex-shrink-0">
              <FolderOpen size={14} className="text-red-400" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-slate-100 font-[Nunito_Sans] truncate group-hover:text-red-400 transition-colors">
                {project.name}
              </p>
              {project.description && (
